...
public class AllAppsQsb extends BaseQsb implements SearchUiManager, OnChangeListener {

    private static final IntProperty<AllAppsQsb> SHADOW_PROGRESS =
            new IntProperty<AllAppsQsb>("shadowProgress") {
                @Override
                public void setValue(AllAppsQsb view, int alpha) {
                    view.setShadowAlpha(alpha);
                }

                @Override
                public Integer get(AllAppsQsb view) {
                    return view.mShadowAlpha;
                }
            };
    ...
    public AllAppsQsb(Context context, AttributeSet attributeSet, int res) {
        ...
        mFixedTranslationY = mContext.getResources().getDimensionPixelSize(R.dimen.qsb_widget_height);
    ...
    public int getMeasuredWidth(int width, DeviceProfile dp) {
        ...
        if (dp.isVerticalBarLayout()) {
            paddingLeft = width - mAppsView.getActiveRecyclerView().getPaddingLeft();
            paddingRight = mAppsView.getActiveRecyclerView().getPaddingRight();
        } else {
            Rect hotseatLayoutPadding = dp.getHotseatLayoutPadding();
            paddingLeft = width - hotseatLayoutPadding.left;
            paddingRight = hotseatLayoutPadding.right;
        }
    ...
    public void setContentVisibility(int visibleElements, PropertySetter setter, Interpolator interpolator) {
        ...
        setter.setFloat(this, SHADOW_PROGRESS, hasAllAppsContent ? 1.0f : 0.0f, Interpolators.LINEAR);
