 ...
 */
package com.paranoid.launcher.qsb.configs;
...
import com.paranoid.launcher.qsb.AllAppsQsbController;
import com.paranoid.launcher.search.nano.SearchProto.Columns;
import com.paranoid.launcher.search.nano.SearchProto.AppIndex;
import com.paranoid.launcher.search.nano.SearchProto.SearchBase;
import com.paranoid.launcher.search.nano.SearchProto.SearchView;
import com.paranoid.launcher.search.AppSearchProvider;
...
public class ConfigurationBuilder {
    ...
    public AllAppsQsbController mAllAppsQsbController;
    public UserManagerCompat mUserManager;

    public ConfigurationBuilder(AllAppsQsbController qsbController, boolean isAllApps) {
        mAllAppsQsbController = qsbController;
        mLauncher = mAllAppsQsbController.mLauncher;
    ...
    public static Intent getSearchIntent(Rect sourceBounds, View gIcon, View micIcon) {
        ...
        return intent.putExtra("source_round_left", true)
                ...
                .setPackage(ParanoidLauncherCallbacks.SEARCH_PACKAGE).addFlags(Intent.FLAG_ACTIVITY_NO_HISTORY | Intent.FLAG_ACTIVITY_NEW_TASK);
    }
    ...
    public final RemoteViews searchQsbTemplate() {
        ...
        int effectiveHeight = ((mAllAppsQsbController.getHeight() - mAllAppsQsbController.getPaddingTop()) - mAllAppsQsbController.getPaddingBottom()) + 20;
        Bitmap mShadowBitmap = mAllAppsQsbController.mShadowBitmap;
        if (mShadowBitmap != null) {
            int internalWidth = (mShadowBitmap.getWidth() - effectiveHeight) / 2;
            int verticalPadding = (mAllAppsQsbController.getHeight() - mShadowBitmap.getHeight()) / 2;
            remoteViews.setViewPadding(R.id.qsb_background_container, mAllAppsQsbController.getPaddingLeft() - internalWidth, verticalPadding, mAllAppsQsbController.getPaddingRight() - internalWidth, verticalPadding);
        ...
        if (mAllAppsQsbController.mMicIconView.getVisibility() != View.VISIBLE) {
            remoteViews.setViewVisibility(R.id.mic_icon, View.INVISIBLE);
        }
        View gIcon = mAllAppsQsbController.findViewById(R.id.g_icon);
        if (mAllAppsQsbController.getLayoutDirection() == View.LAYOUT_DIRECTION_RTL) {
            width = mAllAppsQsbController.getWidth() - gIcon.getRight();
        } else {
            width = gIcon.getLeft();
        }
    ...
    public void createSearchTemplate() {
        ...
        mNano.micIcon = mAllAppsQsbController.mMicIconView.getVisibility() == View.VISIBLE ? R.id.mic_icon : 0;
    ...
    public byte[] build() {
        ...
        mNano.bounds = getViewBounds(mAllAppsQsbController);
