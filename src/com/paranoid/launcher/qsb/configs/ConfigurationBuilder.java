...
import com.paranoid.launcher.qsb.BaseQsb;
...
public class ConfigurationBuilder {
    ...
    public BaseQsb mBaseQsb;
    public UserManagerCompat mUserManager;

    public ConfigurationBuilder(BaseQsb baseQsb, boolean isAllApps) {
        mBaseQsb = baseQsb;
        mLauncher = baseQsb.mLauncher;
    ...
    public final RemoteViews searchQsbTemplate() {
        ...
        int effectiveHeight = ((mBaseQsb.getHeight() - mBaseQsb.getPaddingTop()) - mBaseQsb.getPaddingBottom()) + 20;
        Bitmap mShadowBitmap = mBaseQsb.mShadowBitmap;
        if (mShadowBitmap != null) {
            int internalWidth = (mShadowBitmap.getWidth() - effectiveHeight) / 2;
            int verticalPadding = (mBaseQsb.getHeight() - mShadowBitmap.getHeight()) / 2;
            remoteViews.setViewPadding(R.id.qsb_background_container, mBaseQsb.getPaddingLeft() - internalWidth, verticalPadding, mBaseQsb.getPaddingRight() - internalWidth, verticalPadding);
        ...
        if (mBaseQsb.mMicIconView.getVisibility() != View.VISIBLE) {
            remoteViews.setViewVisibility(R.id.mic_icon, View.INVISIBLE);
        }
        View gIcon = mBaseQsb.findViewById(R.id.g_icon);
        if (mBaseQsb.getLayoutDirection() == View.LAYOUT_DIRECTION_RTL) {
            width = mBaseQsb.getWidth() - gIcon.getRight();
        } else {
            width = gIcon.getLeft();
        }
    ...
    public void createSearchTemplate() {
        ...
        mNano.micIcon = mBaseQsb.mMicIconView.getVisibility() == View.VISIBLE ? R.id.mic_icon : 0;
    ...
    public byte[] build() {
        ...
        mNano.bounds = getViewBounds(mBaseQsb);
