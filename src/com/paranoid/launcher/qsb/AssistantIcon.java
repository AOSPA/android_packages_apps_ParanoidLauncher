...
public class AssistantIcon extends View implements SharedPreferences.OnSharedPreferenceChangeListener {

    public Drawable mIcon;
    public boolean mOpaEnabled;

    public AssistantIcon(Context context) {
        this(context, null, 0);
    }

    public AssistantIcon(Context context, AttributeSet attributeSet) {
        this(context, attributeSet, 0);
    }

    public AssistantIcon(Context context, AttributeSet attributeSet, int res) {
    ...
    public void startService(View view) {
        ...
        if (mOpaEnabled) {
            ...
            if (hasGoogleApp) {
                ...
                isInSession = ActivityManagerWrapper.sInstance.showVoiceSession(null, opt, 5);
            }
            if (isInSession) {
                return;
            }
    ...
    public void updatePrefs(SharedPreferences prefs) {
        ...
        mIcon = getContext().getDrawable(mOpaEnabled ? R.drawable.ic_poodle_color : R.drawable.ic_mic_color);
        update();
    }
    ...
    public void update() {
        Drawable icon = mIcon;
        if (icon != null) {
            int intrinsicWidth = icon.getIntrinsicWidth() / 2;
