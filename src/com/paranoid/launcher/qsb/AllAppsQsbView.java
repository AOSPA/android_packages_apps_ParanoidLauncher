...
import com.paranoid.launcher.search.DefaultSearchView;
...
public class AllAppsQsbView extends BaseQsbView implements SearchUiManager, OnChangeListener {
    ...
    public void setInsets(Rect insets) {
        setSearchType();
        ((MarginLayoutParams) getLayoutParams()).topMargin = Math.max(-mTranslationY, insets.top - mMarginAdjusting);
        requestLayout();
    }
    ...
    public void startSearch(String initialQuery, int result) {
        ...
        if (mLauncher.getCallback().getClient().startSearch(config.build(), config.getExtras())) {
            mLauncher.getCallback().getQsbController().playQsbAnimation();
        } else {
