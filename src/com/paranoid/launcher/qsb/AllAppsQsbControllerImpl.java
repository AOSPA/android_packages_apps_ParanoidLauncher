...
import com.paranoid.launcher.search.SearchHandler;

public class AllAppsQsbControllerImpl extends AllAppsQsbController implements OnClickListener, SearchUiManager, OnChangeListener {
    ...
    public AllAppsQsbControllerImpl(Context context, AttributeSet attributeSet) {
        this(context, attributeSet, 0);
    }

    public AllAppsQsbControllerImpl(Context context, AttributeSet attributeSet, int res) {
    ...
    public void onExtractedColorsChanged(WallpaperColorInfo wallpaperColorInfo) {
        setColor(ColorUtils.compositeColors(ColorUtils.compositeColors(
                ...
                ? R.color.qsb_background_default_dark : R.color.qsb_background_default, 
                Themes.getAttrColor(mLauncher, R.attr.allAppsScrimColor)), wallpaperColorInfo.getMainColor()));
    }
    ...
    public void onClick(View view) {
        if (mLauncher.isInState(ALL_APPS)) {
            ...
            startSearchTask("com.google.android.googlequicksearchbox.TEXT_ASSIST");
        }
    }
    ...
    public void startSearchTask(String action) {
        try {
    ...
    public void warmUpDefaultQsb(String query) {
        if (mAppsSearchContainer == null) {
            initDefaultQsb();
        }
        mAppsSearchContainer.setText(query);
        mAppsSearchContainer.showKeyboard();
    }
    ...
    public void initDefaultQsb() {
        setOnClickListener(null);
        mAppsSearchContainer = (AppsSearchContainerLayout) mLauncher.getLayoutInflater().inflate(R.layout.search_container_all_apps, this, false);
        mAppsSearchContainer.mAllAppsQsb = this;
        mAppsSearchContainer.mApps = mAppsView.getApps();
        mAppsSearchContainer.mAppsView = mAppsView;
        SearchHandler handler = new SearchHandler(mAppsSearchContainer.getContext());
        mAppsSearchContainer.mSearchBarController.initialize(handler, mAppsSearchContainer, Launcher.getLauncher(mAppsSearchContainer.getContext()), mAppsSearchContainer);
        addView(mAppsSearchContainer);
    }

    public void removeDefaultQsb() {
        if (mAppsSearchContainer != null) {
            mAppsSearchContainer.clearSearchResult();
            setOnClickListener(this);
            removeView(mAppsSearchContainer);
            mAppsSearchContainer = null;
        }
    }

    public void resetDefaultQsb() {
        if (mAppsSearchContainer != null) {
            mAppsSearchContainer.reset();
            mAppsSearchContainer.clearSearchResult();
        }
    }
    ...
    protected void setQsbType() {
        boolean useDefaultQsb = !ParanoidUtils.hasPackageInstalled(Launcher.getLauncher(mContext), ParanoidLauncherCallbacks.SEARCH_PACKAGE);
        if (useDefaultQsb != mUseDefaultQsb) {
            ...
            if (mMicIconView != null) {
				mMicIconView.setAlpha(mUseDefaultQsb ? 0.0f : 1.0f);
			}
            if (mUseDefaultQsb) {
                initDefaultQsb();
            }
